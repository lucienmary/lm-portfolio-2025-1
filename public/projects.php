...
<section class="flex relative">
  <div class="container m-auto my-36 px-6 flex flex-col gap-32 xl:gap-36">
    <?php foreach ($projects as $i => $p): ?>
      <?php
      $reverse = ($i % 2) === 1 ? 'md:flex-row-reverse' : '';
      ?>
      <div class="flex flex-col-reverse md:flex-row <?= $reverse ?> gap-6 md:gap-12 xl:gap-24 items-center justify-center">
        <div class="col-span-3 flex flex-col justify-center md:flex-3">
          <span class="font-sans-serif text-sm xl:text-md font-extralight text-gray-400"><?= $p['class'] ?></span>
          ...
          <a href="/project.php?slug=<?= htmlspecialchars($p['slug'], ENT_QUOTES) ?>"
            ...
            Voir le projet
            <svg class="w-[16px] inline fill-white ms-[2px] mt-[-2px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 gap-24 md:flex-4">
          <?php if (!empty($p['image'])): ?>
            <div class="p-[1px] col-span-2 bg-[linear-gradient(to_bottom_left,rgba(255,255,255,1)_1%,rgba(46,151,255,1)_18%,rgba(46,151,255,1)_32%,rgba(255,255,255,1)_50%,rgba(250,0,217,0.88)_68%,rgba(250,0,217,0.88)_82%,rgba(255,255,255,0.75)_100%)] rounded-[calc(theme('borderRadius.2xl')_+_1px)] lg:rounded-[calc(theme('borderRadius.4xl')_+_1px)]">
              <div class="rounded-2xl lg:rounded-4xl w-full h-auto object-cover p-2 lg:p-3 bg-main">
                <a href="/project.php?slug=<?= htmlspecialchars($p['slug'], ENT_QUOTES) ?>">
                  <img src="<?= htmlspecialchars($p['image'], ENT_QUOTES) ?>"
                    alt="<?= htmlspecialchars($p['title'], ENT_QUOTES) ?>"
                    class="shadow-md shadow-gray-800 rounded-xl lg:rounded-3xl">
                </a>
              </div>
            </div>
          <?php endif; ?>
